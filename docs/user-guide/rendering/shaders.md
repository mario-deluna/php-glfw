...
PHP-GLFW makes that log easy to reach. Where OpenGL fills a C buffer through out-parameters, [`glGetShaderInfoLog`](/API/OpenGL/glGetShaderInfoLog.html) and [`glGetProgramInfoLog`](/API/OpenGL/glGetProgramInfoLog.html) simply **return the message as a PHP string**. Here is a small helper that compiles a shader stage and, on failure, puts the GPU's own explanation into the exception:
...
Notice how [`glGetShaderiv`](/API/OpenGL/glGetShaderiv.html) reports back: you pass a `$success` variable and it is filled **by reference**. That is the pattern for every `...iv` query in the API, so `$success` holds the compile status after the call returns.

!!! note "One string, not an array"
    Raw OpenGL's `glShaderSource` takes an array of source strings plus a count. In PHP that is rarely useful, so [`glShaderSource`](/API/OpenGL/glShaderSource.html) is simplified to take a single string. Pass your whole shader as one heredoc.

Linking follows the same shape: create a program, attach the compiled stages, link, then check [`glGetProgramiv`](/API/OpenGL/glGetProgramiv.html) with `GL_LINK_STATUS`.
...
OpenGL is a state machine: you switch the "current" shader on, then every draw call after that uses it. Activate a linked program with [`glUseProgram`](/API/OpenGL/glUseProgram.html) at the top of your render loop, before you set uniforms or draw.
...
Before you can set a uniform you need its location, a small integer handle OpenGL assigns to each named uniform in a linked program. You look it up by name with [`glGetUniformLocation`](/API/OpenGL/glGetUniformLocation.html):
...
Here is where PHP-GLFW is nicer than raw OpenGL. You almost always already have your values as [`Mat4`](/user-guide/math/matrices.html) and `Vec3` objects from the math library, and there are convenience setters that accept those objects directly, with no manual flattening into a buffer:
...
[`glUniformMatrix4f`](/API/OpenGL/glUniformMatrix4f.html) takes a `\GL\Math\Mat4` (the middle argument is the `transpose` flag, normally `GL_FALSE`), and [`glUniformVec3f`](/API/OpenGL/glUniformVec3f.html) takes a `\GL\Math\Vec3`. There are also `glUniformVec2f` and `glUniformVec4f`.
...
When you genuinely need to upload many values at once (an array of bone matrices, a palette of colors), reach for the vector forms like `glUniform3fv` and `glUniformMatrix4fv`. These take a [`FloatBuffer`](/user-guide/buffers/overview.html) or a plain PHP array instead of individual arguments:
...
The other half of a shader's input is its `layout (location = N) in` attributes, the per-vertex data. This is covered end to end in the [Drawing a Triangle](/getting-started/drawing_a_triangle.html) tutorial, so here is just the shape of it and how it ties back to your GLSL.

You upload vertex data with [`glBufferData`](/API/OpenGL/glBufferData.html), which in PHP-GLFW takes a typed buffer object directly (it works out the byte size itself), then describe its layout with [`glVertexAttribPointer`](/API/OpenGL/glVertexAttribPointer.html):
...
For the exhaustive argument list of every function used here, see the generated OpenGL reference: [`glCreateShader`](/API/OpenGL/glCreateShader.html), [`glShaderSource`](/API/OpenGL/glShaderSource.html), [`glCompileShader`](/API/OpenGL/glCompileShader.html), [`glGetShaderiv`](/API/OpenGL/glGetShaderiv.html), [`glGetShaderInfoLog`](/API/OpenGL/glGetShaderInfoLog.html), [`glLinkProgram`](/API/OpenGL/glLinkProgram.html), [`glGetProgramInfoLog`](/API/OpenGL/glGetProgramInfoLog.html), [`glUseProgram`](/API/OpenGL/glUseProgram.html), [`glGetUniformLocation`](/API/OpenGL/glGetUniformLocation.html), and [`glUniformMatrix4f`](/API/OpenGL/glUniformMatrix4f.html).

## See Also

- [Drawing a Triangle](/getting-started/drawing_a_triangle.html) the from-scratch walkthrough of the whole pipeline
- [Textures](/user-guide/rendering/texture2d.html) sampling images inside a fragment shader
- [Matrices](/user-guide/math/matrices.html) building the model, view, and projection matrices you upload as uniforms
- [Buffers](/user-guide/buffers/overview.html) the typed buffers you feed to `glBufferData` and the `...v` uniform forms
