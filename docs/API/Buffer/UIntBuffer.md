# UIntBuffer
The class `GL\Buffer\UIntBuffer` comes with the PHP-GLFW extension to more efficiently 
handle large arrays of `GLuint` values (`int` in php).

**Don't confuse these objects with actual GPU buffers.**

`UIntBuffer` stores an array of real `GLuint` values internally, which saves a lot of memory
...
For ease of use, the `UIntBuffer` constructor accepts a PHP array which will be pushed 
...
The `UIntBuffer` class exposes the following methods to userland.
