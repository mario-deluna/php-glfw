# ShortBuffer
The class `GL\Buffer\ShortBuffer` comes with the PHP-GLFW extension to more efficiently 
handle large arrays of `GLshort` values (`int` in php).

**Don't confuse these objects with actual GPU buffers.**

`ShortBuffer` stores an array of real `GLshort` values internally, which saves a lot of memory
...
For ease of use, the `ShortBuffer` constructor accepts a PHP array which will be pushed 
...
The `ShortBuffer` class exposes the following methods to userland.
