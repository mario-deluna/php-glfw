# IntBuffer
The class `GL\Buffer\IntBuffer` comes with the PHP-GLFW extension to more efficiently 
handle large arrays of `GLint` values (`int` in php).

**Don't confuse these objects with actual GPU buffers.**

`IntBuffer` stores an array of real `GLint` values internally, which saves a lot of memory
...
For ease of use, the `IntBuffer` constructor accepts a PHP array which will be pushed 
...
The `IntBuffer` class exposes the following methods to userland.
